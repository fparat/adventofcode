fn main() {
    ...
    let xmatch = |y: isize, x: isize| -> bool {
        peek(y, x) == Some('A')
            && ((peek(y - 1, x - 1) == Some('M') && peek(y + 1, x + 1) == Some('S'))
                || (peek(y - 1, x - 1) == Some('S') && peek(y + 1, x + 1) == Some('M')))
            && ((peek(y - 1, x + 1) == Some('M') && peek(y + 1, x - 1) == Some('S'))
                || (peek(y - 1, x + 1) == Some('S') && peek(y + 1, x - 1) == Some('M')))
    };
    ...
    for (y, line) in lines.iter().enumerate() {
        for (x, _c) in line.iter().enumerate() {
            part2 += xmatch(y as isize, x as isize) as i32;
        }
    }
