...
from typing import Dict, Iterable, List, Iterator, Optional, Tuple
...
class Node:
    ...
    def items(self, opened_doors=()) -> Iterator[List["Node"]]:
...
cost_cache: Dict[Tuple["State", "State"], int] = {}
next_state_cache = {}
...
class State:
    ...
    y: int
    keys: str = ""
    # next_states: Dict["State", int] = field(default_factory=dict) # possible next states with cost

    def __post_init__(self):
        assert sorted(self.keys) == list(self.keys)
    ...
    def iter_next_states(self, nodes: Dict[Tuple[int, int], Node]) -> Iterator[Tuple["State", int]]:
        for path in nodes[self.x, self.y].keys(self.opened()):
            new_keys = [node.c for node in path if node.c in KEYS and node.c not in self.keys]
            if not new_keys:
                continue # already have all keys
            new_state = State(path[-1].x, path[-1].y, clean_keys(self.keys, new_keys))
            cost = len(path) - 1
            # cost_cache[self, new_state] = cost
...
class PriorityQueue:
    ...
    def get(self, state):
        for qcost, qstate in self.queue:
            if state == qstate:
                return qcost, qstate
        return None, None

    def replace(self, cost, state):
        for idx, (qcost, qstate) in enumerate(self.queue):
            if state == qstate:
                self.queue[idx] = (cost, state)
                break
        self._sort()
