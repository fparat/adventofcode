...
fn main() {
    ...
    let mut all_id = input.lines().into_iter().map(line_id).collect::<Vec<_>>();
    all_id.sort_unstable();
    for try_id in all_id[1]..all_id[all_id.len() - 1] {
        if !all_id.contains(&try_id) {
            println!("Part 2: {}", try_id);
            break;
        }
    }
}
