...
class Position(namedtuple("Position", ["x", "y", "z", "w"])):
    def __repr__(self):
        return f"({self.x:+}, {self.y:+}, {self.z:+}, {self.w:+})"


def iter_pos(x_min, x_max, y_min, y_max, z_min, z_max, w_min, w_max):
    for w in range(w_min, w_max+1):
        for z in range(z_min, z_max+1):
            for y in range(y_min, y_max+1):
                for x in range(x_min, x_max+1):
                    yield Position(x, y, z, w)
            ...
            for dz in range(-1, 2):
                for dw in range(-1, 2):
                    if not (dx == 0 and dy == 0 and dz == 0 and dw == 0):
                        yield Position(pos.x + dx, pos.y + dy, pos.z + dz, pos.w + dw)
...
class PocketDimension:
    ...
    def from_string(cls, s):
        ...
        for row, line in enumerate(s.splitlines()):
            for col, c in enumerate(line.strip()):
                if c == '#':
                    cubes.append(Position(col, row, 0, 0))
        return cls(cubes)
    ...
    def is_active(self, x, y, z, w):
        return Position(x, y, z, w) in self.cubes

    def get_active_zone(self):
        x_min, x_max, y_min, y_max, z_min, z_max, w_min, w_max = 0, 0, 0, 0, 0, 0, 0, 0
        for pos in self.cubes:
            ...
            w_min, w_max = min(w_min, pos.w), max(w_max, pos.w)
        return x_min - 1, x_max + 1, y_min -1, y_max + 1, z_min - 1, z_max + 1, w_min - 1, w_max + 1

    def show(self):
        x_min, x_max, y_min, y_max, z_min, z_max, w_min, w_max = self.get_active_zone()
        for w in range(w_min, w_max+1):
            for z in range(z_min, z_max+1):
                print(f"\nz={z} w={w}")
                for y in range(y_min, y_max+1):
    ...
    def active_neighbors_num(self, pos):
        assert len(list(iter_neighbors(pos))) == 80
...
def main():
    ...
    print(f"Part 2: {pocket.active_cubes_num()} active cubes")
